...
package net.mcreator.darkium.world.dimension;
...
import net.mcreator.darkium.block.SkyWaterBlock;
import net.mcreator.darkium.block.SkyStoneBlock;
...
public class SkyhotWorldDimension extends DarkiumModElements.ModElement {
	@ObjectHolder("darkium:skyhotworld")
	public static final ModDimension dimension = null;
	@ObjectHolder("darkium:skyhotworld_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public SkyhotWorldDimension(DarkiumModElements instance) {
		super(instance, 83);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("skyhotworld"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("darkium:skyhotworld")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("darkium:skyhotworld"), dimension, null, true);
		}
		type = DimensionType.byName(new ResourceLocation("darkium:skyhotworld"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("darkium:darkbiome")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("darkium:skybiome")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("darkium:groly_biome")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new SkyhotWorldItem().setRegistryName("skyhotworld"));
	}
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("skyhotworld_portal");
		}
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, Direction p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos)) || !(this.world.getBlockState(blockpos.down())
							.getBlock() == SkyworldPortalFrameBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = p_180120_1_.offset(p_180120_2_, i);
				return (this.world.getBlockState(framePos).getBlock() == SkyworldPortalFrameBlock.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == SkyworldPortalFrameBlock.block.getDefaultState().getBlock())) {
								break label56;
							}
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutableblockpos,
									flag
											? SkyworldPortalFrameBlock.block.getDefaultState().getBlock().getDefaultState()
											: Blocks.AIR.getDefaultState());
						}
					}
				}
			...
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutableblockpos,
								SkyworldPortalFrameBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new OverworldGenSettings() {
				public BlockState getDefaultBlock() {
					return SkyStoneBlock.block.getDefaultState();
				}

				public BlockState getDefaultFluid() {
					return SkyWaterBlock.block.getDefaultState();
				}
			});
			...
			if (!biomesPatched) {
				for (Biome biome : this.biomes) {
					biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
						{
							carvableBlocks = ImmutableSet.of(SkyStoneBlock.block.getDefaultState().getBlock(),
