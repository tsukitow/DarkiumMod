...
package net.mcreator.darkium.world.biome;
...
import net.mcreator.darkium.block.Darkbiome2stoneBlock;
import net.mcreator.darkium.block.Darkbiome2GrassBlock;
import net.mcreator.darkium.block.DarkBiome2LogBlock;
import net.mcreator.darkium.block.DarkBiome2LeavesBlock;
...
public class GrolyBiomeBiome extends DarkiumModElements.ModElement {
	@ObjectHolder("darkium:groly_biome")
	public static final CustomBiome biome = null;
	public GrolyBiomeBiome(DarkiumModElements instance) {
		super(instance, 107);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.NONE).waterColor(-79902).waterFogColor(-79902)
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(Darkbiome2GrassBlock.block.getDefaultState(),
							Darkbiome2stoneBlock.block.getDefaultState(), Darkbiome2stoneBlock.block.getDefaultState())));
			setRegistryName("groly_biome");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Darkbiome2GrassBlock.block.getDefaultState().getBlock()
							|| ground == Darkbiome2stoneBlock.block.getDefaultState().getBlock())
							&& (ground2 == Darkbiome2GrassBlock.block.getDefaultState().getBlock()
									|| ground2 == Darkbiome2stoneBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), Darkbiome2stoneBlock.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == DarkBiome2LeavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, DarkBiome2LeavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, DarkBiome2LogBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == DarkBiome2LogBlock.block.getDefaultState().getBlock()
					|| blockType == DarkBiome2LeavesBlock.block.getDefaultState().getBlock()
					|| blockType == Darkbiome2GrassBlock.block.getDefaultState().getBlock()
					|| blockType == Darkbiome2stoneBlock.block.getDefaultState().getBlock();
		}
