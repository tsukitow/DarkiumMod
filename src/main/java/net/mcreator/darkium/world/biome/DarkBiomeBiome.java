...
import net.mcreator.darkium.block.DarkWoodBlock;
import net.mcreator.darkium.block.DarkLeavesBlock;
...
public class DarkBiomeBiome extends DarkiumModElements.ModElement {
	@ObjectHolder("darkium:darkbiome")
	public static final CustomBiome biome = null;
	public DarkBiomeBiome(DarkiumModElements instance) {
		super(instance, 40);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.NONE).waterColor(-14352274).waterFogColor(-14352274).parent("plains")
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
				...
				if (!spawnTree) {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == DarkLeavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, DarkLeavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, DarkWoodBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == DarkWoodBlock.block.getDefaultState().getBlock()
					|| blockType == DarkLeavesBlock.block.getDefaultState().getBlock() || blockType == Blocks.GRASS_BLOCK.getDefaultState().getBlock()
